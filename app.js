/* 서울 아파트 실거래 추이 대시보드
 * 데이터: docs/data/transactions_recent.json (최근 1년, rt.molit.go.kr 조건별 자료제공)
 * 컬럼: [구, 동, 단지명, 전용면적(평), 거래금액(만원), 계약일, 층, 건축년도, 유형, 평당금액]
 */
(async function () {
  // ---- 데이터 (최근 1년) ----
  const res = await fetch("data/transactions_recent.json");
  ...
  function updateHero() {
    ...
    document.getElementById("heroPeriod").textContent = meta.period;
    document.getElementById("footerPeriod").textContent = meta.period;
    const at = document.getElementById("aboutTotal");
    if (at) at.textContent = comma(meta.total);
  }
